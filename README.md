...
<!-- 여기에 대시보드 스크린샷을 넣으세요.
     예: ![대시보드](docs/images/dashboard.png) -->
...
- 사용자 몫과 백그라운드 몫을 예비선 방식으로 배분
- 한도 소진 시 버튼을 누르기 전에 미리 차단
- 답변과 추출을 한 번의 호출로 합쳐 메시지당 소모량을 절반으로 줄임
...
| 사례 | 그때 상태 |
|---|---|
| 마감일이 799일 어긋남 | 테스트 265개 전부 통과, 로그 깨끗 |
| 안전장치가 아무것도 안 지킴 | 테스트 315개 전부 통과 |
| 장부는 여유 있다는데 API 한도는 끝남 | 장부 `2/20`, 실제는 소진 |
...
| [기능별 설계 문서](docs/specs/) | 기능마다 배경·설계·범위 밖·알려진 한계 |
| [구현 계획](docs/plans/) | 설계를 작업 단위로 쪼갠 것 |
...
```bash
pytest                            # 백엔드 357개
cd onque-frontend && npm test     # 프론트엔드
```
...
- 화면 디자인 개편 진행 중 — 12개 화면 중 5개 전환 (`feat/ui-direction` 브랜치)
- 미확인 과제: 파일 업로드가 API 한도를 소모하는지 여부
