...
| ID | 날짜 | 영역 | 문제 | 심각도 | 상태 |
|---|---|---|---|---|---|
| TS-029 | 2026-08-10 | FE/레이아웃 | 접힌 목록에 `max-h-[45%]`를 걸었는데 상한이 무효 — 조상 체인에 확정 높이가 없어 `%`가 `aside` 자신의 높이를 기준 삼았고, 그 `aside`가 목록 따라 무한정 자람. 목록이 짧으면 정상으로 보여 tsc·빌드·pytest 어느 것도 못 잡음 | High | 해결됨 |
